...
s1.async=true;
s1.src='https://embed.tawk.to/67be09f37739ba190cd1649c/1ikv5dom5';
s1.charset='UTF-8';
...
export function TawkToWidget() {
  useEffect(
    () =>
      runAfterEngagement(() => {
        if (!document.querySelector("script[data-Muuhu-tawk-position='true']")) {
          const positionScript = document.createElement("script");
          positionScript.dataset.MuuhuTawkPosition = "true";
          positionScript.textContent = tawkToPositionScript;
          document.head.appendChild(positionScript);
        }

        if (!document.querySelector("script[data-Muuhu-tawk='true']")) {
          const widgetScript = document.createElement("script");
          widgetScript.dataset.MuuhuTawk = "true";
