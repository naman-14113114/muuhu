...
export function GiftBundle({ product }: { product: Product }) {
  ...
  return (
    <div>
      ...
      <h1 className="font-serif mt-3 whitespace-nowrap text-[3rem] leading-[1.02] text-[var(--plum)] sm:text-[2.55rem] md:text-[3.25rem] xl:text-[4rem] 2xl:text-[4.45rem]">
        {product.heroTitle}{" "}
      ...
      <div className="mt-3 flex flex-nowrap items-center gap-0.5 sm:gap-1.5 w-full">
        <span className="inline-flex shrink-0 items-center gap-1 sm:gap-1 rounded-full border border-[rgba(58,31,61,.15)] bg-[var(--card)] px-1 sm:px-2 py-1 sm:py-1">
          <ShieldCheck
            size={13}
            strokeWidth={2}
            className="hidden sm:block shrink-0 text-[var(--gold)]"
          />
          <span className="whitespace-nowrap Muuhu-display text-[9px] sm:text-[9.5px] font-bold uppercase tracking-[0em] sm:tracking-[0.02em] text-[var(--plum)]">
            Clinically Proven
          </span>
        </span>
        <span className="inline-flex shrink-0 items-center gap-1 sm:gap-1 rounded-full border border-[rgba(58,31,61,.15)] bg-[var(--card)] px-1 sm:px-2 py-1 sm:py-1">
      ...
      <div className="mt-4 flex flex-col gap-2.5">
        ...
        <div className="flex flex-nowrap items-center gap-x-1 sm:gap-x-1.5 text-[13px] sm:text-[13px] text-[var(--muted)]">
          <span className="whitespace-nowrap tracking-normal">
            or{" "}
            <strong className="Muuhu-display text-[14px] sm:text-[14px] font-medium text-[var(--plum)]">
              4
            </strong>{" "}
            interest-free payments of{" "}
            <strong className="Muuhu-display text-[14px] sm:text-[14px] font-semibold text-[var(--plum)]">
      ...
      <ul className="mt-4 mb-4 flex flex-col gap-1.5 pl-1.5">
        {(product.keyBenefits ?? product.highlights).map((benefit, index) => {
          ...
          return (
            <li
              className="flex items-center gap-3 text-[14.5px] font-normal leading-normal text-[var(--plum)] Muuhu-display"
              key={benefit}
            >
        ...
        <div className="flex items-center justify-between gap-2 sm:gap-5">
          <div>
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <span className="Muuhu-eyebrow text-[var(--gold)] m-0 leading-none flex items-center h-5 sm:h-7 font-bold text-[10px] sm:text-xs">
                FREE DELIVERY
              </span>
            </div>
            <span className="Muuhu-display block mt-1.5 text-base sm:text-2xl text-[var(--plum)] font-normal leading-none whitespace-nowrap">
              {deliveryDate || "soon"}
            </span>
          </div>
          <div className="text-right">
            <span className="Muuhu-eyebrow block text-[var(--gold)] whitespace-nowrap text-[9px] sm:text-[11px] tracking-tight sm:tracking-normal">
              {hasGifts ? "ORDER WITHIN" : "ORDER TODAY"}
            </span>
            <span className="Muuhu-display block mt-1.5 text-xl sm:text-[2.2rem] font-normal text-[var(--plum)] leading-none">
              {timer}
            </span>
          </div>
        </div>
      ...
      <Button
        className="proxy-bundle-btn mt-5 w-full rounded-[30px] border border-[#ED6A3A] bg-[#ED6A3A] py-4 text-xl font-bold uppercase tracking-wide text-[var(--cream)] shadow-lg transition-all duration-300 hover:scale-[1.02] hover:border-[var(--gold)] hover:bg-[#ED6A3A] active:scale-[0.98] sm:text-[22px]"
        id="hero-cta"
        onClick={() => {
          addProduct(product);
          router.push("/cart");
      ...
      {hasGifts ? (
        <section className="mt-8" id="free-gifts">
          <div className="text-center mb-8 flex flex-col items-center">
            <h3 className="Muuhu-display text-3xl font-medium text-[var(--plum)]">
              Special Spring Sale
            </h3>
            <p className="Muuhu-mono mt-2 inline-flex items-center justify-center gap-1.5 flex-wrap rounded bg-[rgba(184,149,86,.15)] px-3 py-1 text-xs sm:text-sm font-bold tracking-widest text-[var(--plum)]">
              <span className="Muuhu-display text-sm sm:text-base font-extrabold normal-case text-[var(--plum)]">
            ...
            {product.gifts.map((gift) => (
              ...
              >
                ...
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-2 sm:px-3 py-1 bg-[var(--card)] border border-[rgba(58,31,61,.22)] rounded-full flex items-center gap-1 sm:gap-1.5 shadow-[0_2px_8px_rgba(58,31,61,0.06)] whitespace-nowrap">
                  ...
                  <span className="line-through text-xs sm:text-sm text-[var(--muted)] Muuhu-display font-semibold leading-none">
                ...
                {/* Gift Label / Title */}
                <p className="Muuhu-display mt-0.5 md:mt-1 text-base md:text-lg font-semibold text-[var(--plum)] leading-snug">
                  {gift.name}
                </p>
              </Link>
            ))}
        ...
        >
          <p className="Muuhu-eyebrow">{product.promoLabel}</p>
          <p className="Muuhu-display mt-2 text-2xl text-[var(--plum)]">
