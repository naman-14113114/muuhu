...
export function StickyAddToCart({ product }: { product: Product }) {
  ...
  useEffect(() => {
    document.documentElement.classList.add("Muuhu-mask-sticky-cta");

    return () => {
      document.documentElement.classList.remove("Muuhu-mask-sticky-cta");
    };
  }, [product.template]);
    ...
    >
      ...
      >
        ...
        <Button
          ...
          className="Muuhu-cart-wipe min-h-11 w-full flex-none px-3 text-[11px] sm:min-h-12 sm:w-auto sm:px-6 sm:text-sm whitespace-nowrap"
          onClick={() => {
            addProduct(product);
            router.push("/cart");
          }}
        >
          {cartIconData ? (
            <div className="Muuhu-sticky-cart-icon flex h-5 w-5 flex-shrink-0 items-center justify-center">
