...
export function CartLineItem({ line }: { line: CartLine }) {
  ...
  return (
    <div className="flex gap-4 border-b border-[var(--border)] py-5">
      {((line.type === "product" && line.slug) || line.title === "Muuhu LED MassageKit") ? (
        <Link
          ...
          href={line.title === "Muuhu LED MassageKit" ? "/products/muuhu-massage-kit" : `/products/${line.slug}`}
        >
          <Image
            alt={line.title}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-3">
          <div>
            <span className="Muuhu-display block text-lg leading-tight text-[var(--plum)]">
              {line.title}
            </span>
            <p className="mt-1 text-xs leading-5 text-[var(--muted)]">{line.subtitle}</p>
          </div>
          <div className="text-right">
            <span className="Muuhu-display block text-lg text-[var(--plum)]">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          {line.locked ? (
            <span className="Muuhu-mono rounded-full bg-[rgba(184,149,86,.12)] px-3 py-1 text-[var(--gold)]">
            ...
            <div className="inline-flex items-center rounded-full border border-[var(--border)] bg-[var(--card)]">
              ...
              <span className="Muuhu-mono min-w-8 text-center text-[var(--plum)]">
                {line.quantity}
              </span>
              <button
