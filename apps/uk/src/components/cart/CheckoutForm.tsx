...
export function CheckoutForm({ initialCustomer }: CheckoutFormProps) {
  ...
  return (
    <>
      ...
      >
        <Button
          className={`relative overflow-hidden w-full rounded-[30px] border border-[#ED6A3A] bg-[#ED6A3A] py-4 text-xl font-bold uppercase tracking-wide text-[var(--cream)] shadow-lg transition-all duration-300 Muuhu-display ${!isRedirecting ? "proxy-bundle-btn" : "disabled:!opacity-100"}`}
          disabled={!hasItems || isRedirecting}
          onClick={handleCheckout}
          type="button"
        >
