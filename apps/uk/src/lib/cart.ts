...
export function getDisplayLines(lines: CartLine[]): CartLine[] {
  const hasMaskProduct = lines.some(
    (line) => line.productId === "muuhu-ipl-hair-removal" && line.type === "product"
  );
    ...
    .map((line) => {
      if (line.productId === "muuhu-ipl-hair-removal" && line.type === "product") {
        return {
          ...
          subtitle:
            "Salon-grade IPL device with the Premium Travel Case included for protected storage and travel.",
          image: "/media/products/muuhu-ipl-hair-removal/images/84-w.webp",
        };
      }
      return line;
    })
    .filter((line) => {
      if (line.productId === "muuhu-ipl-hair-removal" && line.type === "gift") {
