...
export function SpecsPanel({ product }: { product: Product }) {
  return (
    <section className="Muuhu-section bg-[var(--cream)] md: py-14 md:py-24">
      <div className="Muuhu-wrap grid gap-14 lg:grid-cols-2">
        <div>
          ...
          <dl className="mt-8">
            {product.specs.map((spec) => (
              <div
                ...
                key={spec.label}
              >
                <dt className="Muuhu-mono text-[var(--muted)]">{spec.label}</dt>
                <dd className="Muuhu-display text-right text-lg text-[var(--plum)]">
                  {spec.value}
                </dd>
              </div>
            ))}
        ...
        <div>
            ...
            {product.included.map((item) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <span className="Muuhu-mono text-sm text-[var(--gold)]">
                    {item.quantity}
                  </span>
                  <span className="Muuhu-display text-lg text-[var(--plum)]">
                    {item.label}
                  </span>
                </div>
                {item.tag ? (
                  <span className="Muuhu-mono rounded-full bg-[rgba(184,149,86,.24)] px-3 py-1.5 text-[var(--plum)]">
                    {item.tag}
                  </span>
                ) : null}
              </li>
          ...
          <div className="mt-8 rounded-2xl border border-[rgba(58,31,61,.15)] bg-[rgba(241,223,210,.5)] p-6">
            <p className="Muuhu-eyebrow">Product Signals</p>
            <p className="Muuhu-display mt-2 text-xl text-[var(--plum)]">
              {product.badges.join(" · ")}
            </p>
          </div>
        </div>
