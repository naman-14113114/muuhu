const fs = require('fs');
const path = require('path');

const appsDir = path.join(__dirname, 'apps');

const replacements = [
  { search: 'Ã‚Â°', replace: '°' },
  { search: 'Ã¢â‚¬â€œ', replace: '–' },
  { search: 'Ã¢â‚¬â€ ', replace: '—' },
  { search: 'Ãƒâ€”', replace: '×' },
  { search: 'fiancÃ©', replace: 'fiancé' },
  // sometimes degree symbol without the Â
  { search: 'Ã‚°', replace: '°' }
];

function walkDir(dir, callback) {
  if (!fs.existsSync(dir)) return;
  fs.readdirSync(dir).forEach(f => {
    let dirPath = path.join(dir, f);
    let isDirectory = fs.statSync(dirPath).isDirectory();
    if (isDirectory) {
      if (!dirPath.includes('node_modules') && !dirPath.includes('.next')) {
        walkDir(dirPath, callback);
      }
    } else {
      callback(dirPath);
    }
  });
}

let modifiedCount = 0;

walkDir(appsDir, (filePath) => {
  if (filePath.match(/\.(ts|tsx|json)$/)) {
    let content = fs.readFileSync(filePath, 'utf8');
    let originalContent = content;

    replacements.forEach(rep => {
      content = content.split(rep.search).join(rep.replace);
    });

    if (content !== originalContent) {
      fs.writeFileSync(filePath, content, 'utf8');
      modifiedCount++;
      console.log('Fixed encoding in: ' + filePath.replace(appsDir, ''));
    }
  }
});

console.log('\nFinished replacing strings. Modified ' + modifiedCount + ' files.');
